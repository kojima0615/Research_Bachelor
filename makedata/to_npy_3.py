...
def product(data):
    res = np.array([])
    for i in range(data.shape[0]-1):
        res = np.concatenate([res, data[i+1:data.shape[0]] * data[i]])
    return res
...
train_traces = np.fromfile('./trace/AES_TI_wave_mask_v8_0-500000', np.uint8)
...
train_traces = train_traces[:1500000,1200:1700]
print(train_traces.shape)
...
train_traces = np.array(Parallel(n_jobs = 32, verbose=3)([delayed(log_mat)(v) for v in train_traces]))
#train_traces0 = train_traces0.T
#plt.plot(train_traces[0])
#plt.show()
#print(train_traces.shape)
np.save("./trace/AES_TI_wave_mask_log_v8_0-1500000_1200_1700",train_traces)
