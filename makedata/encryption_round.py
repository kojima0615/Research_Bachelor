...
def enc(plain, key):
    ...
    for round in range(9):
        ...
        if round==8:
            p_array = np.zeros((4,4),dtype = int)
            for i in range(4):
                for j in range(4):
                    p_array[i][j] = plain_m1[i][j] ^ plain_m2[i][j]
                    p_array[i][j] = new_base(iso_map_inv,p_array[i][j])
            #print(p_array)
        plain_m1 = add_roundkey(plain_m1, key_index)
    ...
    #print(plain_m1[0][0] ^ plain_m2[0][0])
    #print(plain_m1[0][1] ^ plain_m2[0][1])
    key_index = expand_key(key_index, 10)
    #print(key_index)

    #最後のadd_roundkeyの前後でレジスタに詰めてたから取ってみる
    #ak_before = plain_m1[0][0] ^ plain_m2[0][0]
    plain_m1 = add_roundkey(plain_m1, key_index)
    #key_10 = key_index[0][0]
    #ak_after = plain_m1[0][0] ^ plain_m2[0][0]
    #round10 = plain_m1[0][0] ^ plain_m2[0][0]
    #逆変換
    ...
    return key_index[0][1], res_m[0][1]
