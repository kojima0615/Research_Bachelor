...
'''
x_memmap = np.memmap(
filename=X_MEMMAP_PATH, dtype=np.float, mode='r', shape=(data_size, wave_size))
wave = x_memmap[:]
#data_size = 400000
def calc_corr(w, hs):
...
def calc_sc(w, hs, key_true):
    out = np.zeros((300000//3000, 256 * 256) ,dtype = np.float32)
    indices = np.arange(hs.shape[1])
    indices = np.random.permutation(indices)
    indices = indices[:300000]
    wt = w[indices]
    hst = hs[:, indices] 
    for i in range(out.shape[0]):
        out[i] = [abs(np.corrcoef([wt[:(i+1) * 3000], hst[key,:(i+1)*3000]])[0][1]) for key in range(256)]
...
def calc_sc_twov(w, hs, key_true):
    out = np.zeros((200000//10000, 256 * 256) ,dtype = np.float32)
    indices = np.arange(hs.shape[1])
    indices = np.random.permutation(indices)
    indices = indices[:210000]
    wt = w[indices]
    hst = hs[:, indices]
...
def main():
    ...
    r) for key in range(256)]), dtype = np.float)
    '''
    #2byte予測
    ...
    #corr_out = np.array(Parallel(n_jobs=64, verbose=5)([delayed(calc_corr_cpa_memmap)(i, hs) for i in range(wave.shape[1])]))
    ...
    np.save('./csv_data/corr_0225_mask_TI_product_v8', corr_out)
    corr_out = corr_out.T
    ...
    plt.show()
    plt.savefig("./image/corr_0225_mask_TI_product_v8.png")
    '''
    
    #success rate
    ave = 100
    outs = np.zeros((ave, 200000//10000),dtype=np.int64)
    #outs = np.array(Parallel(n_jobs=2, prefer='threads', verbose=5)([delayed(calc_sc_twov)(w, hs, key_true) for _ in range(ave)]))
    for i in range(ave):
        outs[i] = calc_sc_twov(w, hs, key_true)
    del hs
    sc = np.zeros(200000//10000)
    np.save('./csv_data/cpa_0215_mask_TI_time_SC_v8_twovalue', outs)
    for i in range(ave):
        sc += (outs[i]==0)
        print(i)
    sc /= ave
    plt.plot(sc)
    plt.savefig("./image/cpa_0215_mask_TI_time_SC_v8_twovalue.png")
