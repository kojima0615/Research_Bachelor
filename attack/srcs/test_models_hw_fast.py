...
def model_fit(model, X_attack):
    #index = 0最初の入力のshape
    input_layer_shape = model.get_layer(index = 0).input_shape
    
    input_data = X_attack
    if len(input_layer_shape) == 3:#3次元入力
        #2次元にreshape
...
def rank_key(X,pt):
    ...
    for i in range(X.shape[0]):
        for key in range(256):
            ...
            outs = hd_TI(key, pt[i])
            likelihood[i][key] += np.log(X[i][outs])
            if i!=0:
...
def loop(i,preds,plaintext, key, index):
    ...
    from rank_256_beforeak import rank_fast_256_beforeak
    out = rank_fast_256_beforeak(X, pt)
    '''
    plt.plot(out)
    plt.grid()
    plt.show()
...
if __name__=="__main__":
    ...
    _, (X_attack, _) = load_data.traces(dataset,model_file)
    ...
    print("model_fit")
    preds = model_fit(model, X_attack)
    del X_attack
    del model
    print(preds.shape)
    ...
    outs = Parallel(n_jobs = 4, verbose=3)([delayed(loop)(i, preds[0], plaintext, key, index)for i in range(args.average)])
