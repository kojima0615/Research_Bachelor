...
def rank_nonshare(X, pt, r):
    ...
    likelihood = np.zeros((X.shape[0],256),dtype = np.float32)
    for i in range(X.shape[0]):
        for key in range(256):
            #iso
            '''
            ...
            out = inv_s_box[out]
            out = out ^ r[i]
            likelihood[i][key] += np.log(X[i][out], dtype = np.float32)
            if i!=0:
                likelihood[i][key] += likelihood[i-1][key]
    for i in range(X.shape[0]):
        likelihood[i] /= i+1
...
@jit('f4[:,:](f4[:,:],i8[:],i8[:])', nopython=True, parallel = True)
def rank_twov(X, pt1, pt5):
    #nonisomap    
    likelihood = np.zeros((X.shape[0],256 * 256), dtype=np.float32)
    for i in range(X.shape[0]):
        for key1 in prange(256):
            for key5 in range(256):
                key15 = key1 + (key5 << 8)
                #out = hw[inv[pt1[i] ^ key1] ^ inv[pt5[i] ^ key5]]
                out = inv[pt1[i] ^ key1] ^ inv[pt5[i] ^ key5]
                likelihood[i][key15] += np.log(X[i][out]) #- np.log(pdf[out])
                if i!=0:
...
@jit('i8[:](f4[:,:],i8,i8)', nopython=True, parallel = True)
def parallel_argsort(preds, key1, key5):
    out = np.zeros(preds.shape[0], dtype=np.int64)
    key15 = key1 + (key5 << 8)
    for i in prange(preds.shape[0]//100):
        for j in range(100):
            k = 100 * i + j
            out[k] = 65280 + 255 - np.argsort(np.argsort(preds[k]))[key15]
    return out
def loop_twov(i, preds, PT1, PT5, key1, key5):
    ...
    out = parallel_argsort(out, key1, key5)

    #out = 65280 + 255 - np.argsort(np.argsort(out))[:,key1 + (key5 << 8)].ravel()
...
if __name__=="__main__":
    ...
    preds = np.load(add_prefix(fig_name, "preds"))
    ...
    #nonisomap_twovalue
    outs = Parallel(n_jobs = 1, verbose=3)([delayed(loop_twov)(i, preds, \
        plaintext, plaintext5, key, key5)for i in range(args.average)])
    ...
    '''
    print(ge)
    np.save(add_prefix(fig_name, "GEtwov"), ge)
    np.save(add_prefix(fig_name, "SCtwov"), sc)
