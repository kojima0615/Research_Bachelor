...
from tensorflow.keras.layers import Flatten, Dense, Input, Conv1D, AveragePooling1D, MaxPooling1D, BatchNormalization, Activation, Layer,Dense,GaussianNoise
...
def scnn_9_elu_symmetric_32_firstdense(input_size=700, classes=256):
    input_shape = (input_size,1)
    model = k.Sequential([
        kl.Input(shape=input_shape),
        kl.Flatten(),
        kl.Dense(input_size, kernel_initializer='he_uniform', activation='selu'),
...
def scnn_9_elu_symmetric_32_cnn11(input_size=700, classes=256):
    input_shape = (input_size,1)
    model = k.Sequential([
        kl.Input(shape=input_shape),
...
def scnn_9_elu_symmetric_32_cnn13(input_size=700, classes=256):
    input_shape = (input_size,1)
    model = k.Sequential([
        kl.Input(shape=input_shape),
        *get_block(32, 'SYMMETRIC', 'selu'),
...
def scnn_9_elu_symmetric_64_cnn13(input_size=700, classes=256):
    input_shape = (input_size,1)
    model = k.Sequential([
        kl.Input(shape=input_shape),
        *get_block(64, 'SYMMETRIC', 'selu'),
...
def scnn_9_elu_symmetric_16_cp(input_size=700, classes=256, classes_1 = 256):
    input_shape = (input_size,1)
    ...
    output2 = kl.Dense(classes_1, kernel_initializer='he_uniform', name='output2')(y)
...
def scnn_9_elu_symmetric_16_twoinput(input_size=700, classes=256):
    input_shape = (input_size,1)
    input_w = kl.Input(shape=input_shape)
    input_B = kl.Input(shape = (classes))
    y = kl.Dense(4, kernel_initializer='he_uniform', activation='selu')(input_B)
    x = get_block_twov(16, 'SYMMETRIC', 'selu',input_w)
    x = get_block_twov(16, 'SYMMETRIC', 'selu',x)
    x = get_block_twov(16, 'SYMMETRIC', 'selu',x)
    x = get_block_twov(32, 'SYMMETRIC', 'selu',x)
    ...
    x = Activation('selu')(x)
    x = kl.GlobalMaxPooling1D()(x)
    x =  kl.Dense(128, kernel_initializer='he_uniform', activation='selu')(x)
    x = kl.concatenate([x, y])
    output1 = kl.Dense(classes, kernel_initializer='he_uniform', name='output1')(x)
...
def zaid_ASCAD(size, classes = 256):
...
def zaid_ASCAD_maxpooling(size, classes = 256):
	# Designing input layer
    model = k.Sequential()
    model.add(kl.Input(shape=(size,1)))
    # 1st convolutional block
    ...
    model.add(kl.BatchNormalization())
    model.add(kl.Activation('selu'))
    model.add(kl.MaxPooling1D(2, strides=2))
    model.add(kl.Flatten())
    # Classification layer
    model.add(kl.Dense(20, kernel_initializer='he_uniform', activation='selu'))
    model.add(kl.Dropout(0.5))
...
def CNN4_8_v2(size, classes = 256):
	# Designing input layer
    model = k.Sequential()
    model.add(kl.Input(shape=(size,1)))
    # 1st convolutional block
    ...
    model.add(kl.Activation('selu'))
    model.add(kl.AveragePooling1D(2, strides=2))
    model.add(kl.Conv1D(64, 3, kernel_initializer='he_uniform', padding='same'))
    model.add(kl.BatchNormalization())
    model.add(kl.Activation('selu'))
    model.add(kl.AveragePooling1D(2, strides=2))
    model.add(kl.Flatten())
...
def CNN5_64(size, classes = 256):
    ...
    model.add(kl.Activation('selu'))
    model.add(kl.AveragePooling1D(2, strides=2))
    model.add(kl.Conv1D(128, 3, kernel_initializer='he_uniform', padding='same'))
    model.add(kl.BatchNormalization())
    model.add(kl.Activation('selu'))
    model.add(kl.AveragePooling1D(2, strides=2))
    model.add(kl.Conv1D(128, 3, kernel_initializer='he_uniform', padding='same'))
    ...
    model.add(kl.Flatten())
    # Classification layer
    model.add(kl.Dense(128, kernel_initializer='he_uniform', activation='selu'))
    model.add(kl.Dense(128, kernel_initializer='he_uniform', activation='selu'))
    # Logits layer
    model.add(kl.Dense(classes))
    return model
    
def CNN5_16(size, classes = 256):
	# Designing input layer
    model = k.Sequential()
    model.add(kl.Input(shape=(size,1)))
    # 1st convolutional block
...
def CNN10_8_v2(size, classes = 256):
	# Designing input layer
    model = k.Sequential()
    model.add(kl.Input(shape=(size,1)))
    # 1st convolutional block
